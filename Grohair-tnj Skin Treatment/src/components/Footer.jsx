...
const quickLinks = [
  { name: 'Home', link: '#hero' },
  { name: 'Skin Problems', link: '#facing' },
  { name: 'Skin Solutions', link: '#treatments' },
  { name: 'Before & After', link: '#results' }
]
const services = [
  { name: 'Laser Skin Resurfacing', link: '#treatments' },
  { name: 'Microneedling', link: '#treatments' },
  { name: 'Chemical Peels', link: '#treatments' },
  { name: 'PRP Skin Rejuvenation', link: '#treatments' }
]

const MAPS_URL = 'https://www.google.com/maps/search/?api=1&query=3,+1st+Floor,+Philomena+Hotel+and+Apartment,+Arulanandha+Nagar+Main+Rd,+Arulananda+Nagar+West+Extension,+Thanjavur,+Tamil+Nadu+613007'
...
export default function Footer() {
  return (
    <footer id="footer" className="bg-[#0f1117]">
      ...
      {/* ── Logo + tagline ── */}
      <div className="px-5 pt-7 pb-5 border-b border-white/10 flex items-center gap-6">
        <img src={footerImg} alt="Gloskin" className="h-12 w-auto object-contain" />
        <p className="text-[14px] text-white/35 leading-relaxed font-semibold">Advanced Gloskin<br />Arulanandha Nagar, Thanjavur</p>
      </div>
      ...
      <div className="px-5 py-5 border-b border-white/10 flex flex-col gap-4">
        ...
        >
          ...
          <span className="text-[12px] text-white/45 leading-relaxed hover:text-red-400 active:text-red-400 transition-colors">
            3, 1st Floor, Philomena Hotel and Apartment, Arulanandha Nagar Main Rd, Arulananda Nagar West Extension, Thanjavur, Tamil Nadu 613007
          </span>
        </a>

        {/* Phone → Dialer */}
        <a href="tel:+918098756789" className="flex items-center gap-3 group">
          ...
          <span className="text-[12px] text-white/45 hover:text-red-400 active:text-red-400 transition-colors">80987 56789</span>
        </a>

        {/* Email → Mail app */}
        <a href="mailto:grohairgloskintnj@gmail.com" className="flex items-center gap-3 group">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" className="w-4 h-4 text-red-500 shrink-0">
            <path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/>
            <polyline points="22,6 12,13 2,6"/>
          </svg>
          <span className="text-[12px] text-white/45 hover:text-red-400 active:text-red-400 transition-colors">grohairgloskintnj@gmail.com</span>
        </a>
      ...
      <div className="px-5 py-4 text-center">
        <p className="text-[11px] text-white/20">© 2026 GloSkin. All rights reserved.</p>
      </div>
    </footer>
  )
}
